    ...
    return (
        <div>
            ...
            <button 
                ...
                id="«Start»"
                onClick={() => {
                    startHandler({ setCount, timer, setTimer, render, isStarted, setIsStarted })
                }}
            >
                «Start» &#8260; «Stop»
            </button>
...
const startHandler = ({ render, isStarted, setIsStarted }) => {
    if (isStarted) {
    ...
    const timerFlow = merge(
        ...
        waitBtn$.pipe(
            mapTo((acc) => {
                if (isDoubleclick({timerFlow})) {
                    timerFlow.unsubscribe();
                    setIsStarted(false);
                };
                return acc;
            })
